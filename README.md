...
동봉되지 않은 타임프레임은 **고른 기간에 필요한 만큼** 실행 시 Upbit API로 받아온 뒤
`data/ohlcv/` 에 parquet으로 저장합니다. 두 번째 실행부터는 API를 치지 않습니다.
(이 파일들은 `.gitignore` 에 걸려 있어 저장소 크기에 영향을 주지 않습니다.)

> 최초 수집은 1h·1년 기준 9개 코인에 약 60초 걸립니다. 진행 중에는 안내가 표시됩니다.

Upbit이 해외 IP를 차단하는 경우가 있어 클라우드 환경에서는 실패할 수 있습니다.
그때는 앱이 안내를 띄우고 일봉으로 돌아가도록 되어 있습니다.

**요청한 기간을 채우지 못하면 화면에 경고가 뜹니다.** 예전에는 "최근 1년"을 골라도
1,000봉(42일)만 분석되면서 아무 표시가 없었고, 그 짧은 표본에서 XRP의 k=+1 R²가
0.49%로 "유의"하게 나온 적이 있습니다. 표본을 8,735봉으로 늘리자 0.0006%(p=0.82)로
사라졌습니다 — 짧은 표본이 만들어낸 노이즈였습니다.
