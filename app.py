...
# ======================================================================
# 6. Lead-Lag — 기준 코인을 보고 따라 사면 되나
# ======================================================================

st.markdown(f"### 6. Lead-Lag — {market_label}를 보고 따라 사면 되나")

st.markdown(
    f"""
    지금까지의 회귀는 **같은 봉**끼리 맞춘 것이라 *설명*이지 *예측*이 아닙니다.
    여기서는 {market_label} 수익률을 **k봉 밀어서** 회귀합니다.

    $$r_{{i,t}} = \\alpha + \\beta \\cdot r_{{m,\\,t-k}} + \\varepsilon_t$$

    - **k > 0** — {market_label}가 **선행**. 값이 크면 *"{market_label}를 보고 나서 사도 늦지 않다"*
    - **k = 0** — 동시점. 앞 섹션들이 재던 것
    - **k < 0** — 이 코인이 {market_label}보다 **먼저** 움직임
    """
)

ll1, ll2 = st.columns([2.4, 1])
with ll1:
    ll_picked = st.multiselect(
        "코인 (최대 8개)", list(results.keys()),
        default=list(results.keys())[:4], format_func=short, key="ll_coins",
    )
with ll2:
...
def lead_lag_profile(y_vals: tuple, x_vals: tuple, max_lag: int) -> pd.DataFrame:
    """기준 코인을 k봉 밀어가며 회귀. k>0이면 기준 코인이 선행."""
    y = pd.Series(y_vals)
    x = pd.Series(x_vals)
    rows = []
...
if ll_picked:
    ...
    for m in ll_picked:
        pair = rets[[MARKET, m]].dropna()
        profiles[m] = lead_lag_profile(
            tuple(pair[m].values), tuple(pair[MARKET].values), max_lag
        )
    ...
    for m, prof in profiles.items():
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=(f"<b>{short(m)}</b><br>k=%{{x}}봉<br>"
                           "R² = %{y:.3f}<extra></extra>"),
        ))
    fig.add_vline(x=0, line=dict(color=MUTED, width=1))
    fig.add_annotation(x=0, y=1.02, yref="paper", text="k=0 (동시점)",
                       showarrow=False, font=dict(color=MUTED, size=10), xanchor="left")
    fig.update_xaxes(title_text=f"k — {market_label}를 몇 봉 밀었나  (오른쪽 = {market_label} 선행)",
                     dtick=1)
    ...
    for m, prof in profiles.items():
        ...
        if p0.empty or p1.empty:
            continue
        r2_0 = float(p0["r2"].iloc[0])
        r2_1 = float(p1["r2"].iloc[0])
        pv_1 = float(p1["p"].iloc[0])
        drop = (1 - r2_1 / r2_0) if r2_0 > 0 else np.nan

        # 예측 가능한 1봉 움직임의 크기 vs 왕복 수수료
        sigma_bar = float(rets[m].std(ddof=1))
        edge = math.sqrt(max(r2_1, 0.0)) * sigma_bar
    ...
    st.dataframe(
        ...
        column_config={
            "k=0 R²": st.column_config.NumberColumn(
                format="%.1f%%", help="같은 봉끼리 맞췄을 때의 설명력"),
            "k=+1 R²": st.column_config.NumberColumn(
                format="%.2f%%", help="기준 코인을 한 봉 밀었을 때 남는 설명력"),
            "설명력 감소": st.column_config.NumberColumn(
                format="%.1f%%", help="k=0 대비 몇 % 사라졌나"),
            "k=+1 p값": st.column_config.NumberColumn(
                format="%.3f", help="0.05 미만이면 우연이 아니라고 본다"),
    ...
    if not cmp_df.empty:
        ...
        st.markdown(
            ...
            | 최고 예측가능 변동폭 | **{cmp_df['예측가능 변동폭'].max():.3f}%** | 왕복 수수료 {ROUND_TRIP_COST:.2f}% 와 비교하세요 |
            """
        )
        ...
        if edge_max < ROUND_TRIP_COST:
            st.error(
                ...
                f"이 구간에서 '{market_label}를 보고 따라 사는' 전략은 성립하지 않습니다."
            )
        else:
            st.warning(
    ...
    st.info(
        ...
        f"**{(2 * max_lag + 1) * len(profiles)}번의 검정**을 동시에 하고 있습니다. "
        f"유의수준 5%면 아무 관계가 없어도 우연히 "
        f"**{round((2 * max_lag + 1) * len(profiles) * 0.05)}개 정도**는 'p<0.05'로 나옵니다. "
